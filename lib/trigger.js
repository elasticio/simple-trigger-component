...
async function simpleTrigger(msg, cfg, snapshot = {}) {
  ...
  if (now < new Date(cfg.startTime).getTime()) {
    if (isDebugFlow) throw new Error('"Start time" is greater then current time, no messages will be produced. This error is only applicable to the Retrieve Sample. In flow executions there will be no error, executions will be skipped until Start Time becomes greater than current time.');
